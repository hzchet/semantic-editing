...
from .utils import get_blocks, bottleneck_IR, bottleneck_IR_SE, l2_norm
...
class Backbone(nn.Module):
    def __init__(self, input_size, num_layers, mode="ir", drop_ratio=0.4,
        ...
        self.input_layer = nn.Sequential(
            nn.Conv2d(3, 64, (3, 3), 1, 1, bias=False), nn.BatchNorm2d(64),
            nn.PReLU(64)
        )
        if input_size == 112:
        ...
        for block in blocks:
            for bottleneck in block:
                modules.append(
                    unit_module(
                        bottleneck.in_channel, bottleneck.depth,
                        bottleneck.stride
                    )
                )
